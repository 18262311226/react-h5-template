...
export default function TplSide () {
    ...
    return <div className={classNames(leftsidestyles.main)}>
        <ul className={classNames(leftsidestyles.box)}>
            {settings.map(item => (
                <li className={classNames(leftsidestyles.item)} key={item.key} onClick={() => setCanvas(item.data)}>
                    <div className={classNames(leftsidestyles.desc)}>{item.desc}</div>
